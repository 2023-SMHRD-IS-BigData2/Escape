...
public class Game_main {
	...
	public static void main(String[] args) {
		...
		if (ch == 1) {
			...
			String[] lifeMsg = { "♥", "♥♥", "♥♥♥", "♥♥♥♥" };
			int[] codes = { 1, 2, 3, 4 };
			difficulty = new Gameeasy(4, lifeMsg, codes, 4);
			con = new Controller(difficulty);
			con.Play();
		} else if (ch == 2) {
			pe.D = 6;
			gf.mode = 2;
			...
			difficulty = new Gameeasy(6, lifeMsg, codes, 6);
			con = new Controller(difficulty);
			con.Play();
		} else if (ch == 3) {
			pe.D = 8;
			...
			difficulty = new Gameeasy(8, lifeMsg, codes, 8);
			con = new Controller(difficulty);
			con.Play();
		}
